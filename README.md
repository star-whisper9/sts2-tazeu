...
- **一对多连接**：支持同时连接多台郊狼 APP（1 人游玩，N 人被电）
- 游戏内可视化客户端管理面板（Kick / Block）
- _多人游戏适配\*_
...
1. ~~\* 多人模式中，所有鸡煲的电球都能触发电击（Hook 未区分玩家）。~~ 已支持"仅自己的电球"开关。
...
5. ! Block 功能基于客户端 IP 地址实现（会话级，不持久化）。若在反向代理/内网穿透后使用，所有客户端可能共享同一 IP，导致 Block 一个客户端会误伤其他客户端，或 Block 完全失效。此限制由 DG-LAB WebSocket 协议决定，无法在 Mod 侧解决。
...
连接成功后，关闭再召出二维码将能看到绿色的已连接状态和已连接的客户端列表。**修改连接端口需要手动重新刷新二维码**。

**一对多模式**：多个 DG-LAB APP 可以同时扫描同一个二维码连接。每个 APP 独立绑定到各自的郊狼硬件，电击事件会广播到所有已绑定的客户端。在二维码弹窗中可以查看所有已连接的客户端，并通过 Kick（踢出）或 Block（屏蔽，本次会话内禁止重连）管理连接。
...
- **A/B通道**：可单独切换郊狼通道是否启用
- **仅自己的电球**：开启后只有自己的闪电充能球触发电击，队友的电球不会影响你（多人模式适用）
...
- **一对多连接架构**：使用 `ConcurrentDictionary<string, AppClient>` 维护客户端列表，每个客户端持有独立的 `WebSocket`、`SemaphoreSlim` 发送锁和通道强度上限。电击事件触发时共享 Combo 计数器和波形选择，但各客户端按自身通道上限独立映射强度后并发发送。屏蔽列表基于 IP 地址的 `HashSet`（会话级，不持久化），受 `lock` 保护。
